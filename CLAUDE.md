...
- **Free first:** search goes to the organization's own SearXNG and pages are read by `acquisition/web.py` (direct fetch plus trafilatura). Pages and feeds are fetched through `acquisition/fetching.py` (`SafeFetcher`): non-public addresses refused at every redirect, robots.txt respected, bodies capped, and a deadline for the whole fetch (`asyncio.timeout` around httpx's async client; from outside a blocking call). Tavily is the paid fallback for both: pages the web reader cannot read (never social media, forums or Moltbook: `NOT_WORTH_PAYING`), and searches SearXNG fails or finds nothing for. When SearXNG's engines are blocked (`SearchUnavailable`: no results and unresponsive engines), nothing is paid instead: the job waits 30 minutes without using an attempt. `searxng.py` leaves 3 seconds between queries, and `searxng/settings.yml` spreads them over Bing, Mojeek, Yahoo, Qwant, Brave and DuckDuckGo.
